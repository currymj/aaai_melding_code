...
from kidney_solver_local import kidney_digraph
...
# the bottom half of A will still be an identity matrix like before
# the top half of A should be one constraint per node, each row one-hot
# for the cycles that node is in. i.e. almost the same thing as here,
# just not complete

# this means that the matching matrix needs access to the digraph, same
# as with trimble's code. or at least a list of all the 3-cycles
# and not chains

def make_matching_matrix(graph, cycles, nodes_to_cycles, max_length=3):
    # precompute cycles to ensure consistent order
    #cycles = graph.find_cycles(max_length)

    # one var per cycle
    n_vars = len(cycles)

    # one constraint per node, plus cycle vars must be btwn 0,1
    n_constraints = len(graph.vs) + len(cycles)
    ...
    # for each node, flip its cycles on
    for constraint_ind in range(0, len(graph.vs)):
        b[constraint_ind] = 1.0
        for cycle in nodes_to_cycles[constraint_ind]:
            A[constraint_ind, cycle] = 1.0

    # for each cycle, ensure it is >0
    for val in range(n_vars):
        constraint_ind = len(graph.vs)+val
        A[constraint_ind, val] = -1.0
        
    return A, b

def make_matching_matrix_old(n):
...
if __name__ == '__main__':
    graph = kidney_digraph.Digraph(10)
    graph.add_edge(1.0,graph.vs[0], graph.vs[9])
    graph.add_edge(0.5,graph.vs[0], graph.vs[1])

    
    with open('MD-00001-00000100.input') as graphfile:
        input_lines = [line for line in graphfile if len(line.strip()) > 0]
